...
Tensor sum_dim_IntList(
  ...
  if (opt_dim.has_value()) {
    ...
    for (const auto& dim : dims) {
      // Do dim check before normalization to report to specified wrong dim value to user
      TORCH_CHECK(
      ...
      dims_set.insert(utils::normalize(dim, self.dim()));
    }
    Tensor result = self;
    // Reduce the higher dimensionalities first, otherwise when keepdim is false, it will
    // be reducing the wrong dimension.
...
TORCH_LIBRARY_IMPL(aten, Vulkan, m) {
  ...
  m.impl(
      TORCH_SELECTIVE_NAME("aten::sum"), TORCH_FN(sum));
}
